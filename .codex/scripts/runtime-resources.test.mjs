...
test("composeCommand pins project, file, loopback database port, and ownership variables", () => {
  ...
  assert.deepEqual(command.args, [
    ...
    "-f",
    `${allocation.root}/.codex/runtime/compose.yaml`,
    "up",
    "-d",
    "postgres",
  ]);
...
test("Docker inspection fails closed when Docker is unavailable or a label differs", async () => {
  assert.equal(
    (await inspectOwnedDockerResources(allocation, dockerFixture({ unavailable: true }))).code,
    "docker-unavailable",
  );
  assert.equal(
    (await inspectOwnedDockerResources(allocation, dockerFixture({ mutateLabel: true }))).code,
    "docker-label-mismatch",
  );
  assert.equal(
    (await inspectOwnedDockerResources(allocation, dockerFixture({ missingLabel: true }))).code,
    "docker-label-mismatch",
  );
  assert.equal(
    (await inspectOwnedDockerResources(allocation, dockerFixture({ foreignProject: true }))).code,
    "docker-label-mismatch",
  );
});
...
test("Docker removal never mutates after mismatch and otherwise removes containers, networks, then volumes", async () => {
  ...
  assert.equal(
    mismatch.calls.some((call) => call.includes("rm")),
    false,
  );
