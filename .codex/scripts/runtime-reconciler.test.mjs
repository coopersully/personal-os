...
test("classification protects live, present-unregistered, and missing-locked roots", () => {
  ...
  assert.equal(
    classifyAllocation(allocation, { rootExists: true, worktree: registered }, now).kind,
    "live",
  );
  assert.equal(
    classifyAllocation(allocation, { rootExists: false, worktree: locked }, now).kind,
    "drifted-missing-locked",
  );
  assert.equal(
    classifyAllocation(allocation, { rootExists: true, worktree: null }, now).kind,
    "drifted-present-unregistered",
  );
});
...
test("orphan cleanup requires two observations and at least sixty seconds", () => {
  ...
  assert.equal(
    classifyAllocation({ ...allocation, orphanedAt: "2026-08-27T12:00:01.000Z" }, missing, now)
      .kind,
    "orphan-pending",
  );
  assert.equal(
    classifyAllocation({ ...allocation, orphanedAt: "2026-08-27T12:00:00.000Z" }, missing, now)
      .kind,
    "orphan-ready",
  );
  assert.equal(
...
test("first missing observation marks pending and a reappearing root clears it", async () => {
  ...
  await reconcileAllocation(
    {},
    { ...allocation, orphanedAt: now.toISOString() },
    {
      ...recovery,
...
test("cleanup failure retains the allocation and reserved tier", async () => {
  const fixture = adapters({
    removeOwnedDockerResources: async () => ({ ok: false, code: "docker-label-mismatch" }),
  });
  const result = await reconcileAllocation(
    {},
    {
  ...
  assert.equal(
    fixture.mutations.some(([action]) => action === "delete"),
    false,
  );
});
...
test("an operation-token mismatch cannot delete a newer allocation", async () => {
  ...
  assert.equal(
    fixture.mutations.some(([action]) => action === "replace" && action === "cleanup-failed"),
    false,
  );
});
