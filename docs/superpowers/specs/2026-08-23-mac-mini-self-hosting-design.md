...
The checked-in profile sets CPU, memory, data-disk, root-disk, architecture, and runtime. Bootstrap verifies the effective allocation inside the VM and leaves at least 25% host memory and 20% host disk free. A root-owned host-runtime compatibility manifest records the tested macOS build, Colima, Lima, virtualization backend, Docker Engine, Compose, profile schema, and launchd definitions. It is part of the installed fixed-code/trust set; bootstrap rejects drift from that manifest, and a candidate-bundle copy cannot select the verifier or Compose version used to validate itself.
...
- an infrastructure-change class plus attested rehearsal approval when PostgreSQL, `cloudflared`, or the fixed-code set changes.

The protected-main `push` workflow binds CI and release to the same event SHA and workflow definition: the release job has `needs` on every required CI job and runs only for that successful `main` push. It does not use `workflow_run`, `repository_dispatch`, or a later default-branch checkout to release an older run. Therefore A's delayed success still executes and attests A's reviewed workflow at A even if newer B has landed and failed. Actions generates artifact attestations for the bundle and application images. Only the release job receives package publication privileges. An infrastructure-class release also enables a separate same-run `production-infrastructure` environment job with `contents: read`, Actions/environment read access, `attestations: write`, and `id-token: write`; it has no package publication, environment administration, production runtime secret, or follow-on dispatch capability. Every job pins third-party actions to reviewed commits, and the release job rebuilds from the event SHA without consuming untrusted artifacts or caches.
...
An ordinary application release must keep the PostgreSQL, `cloudflared`, and fixed-code/trust-set digests unchanged. A changed infrastructure digest requires a distinct attested infrastructure release, successful clean restore and upgrade rehearsal, explicit Cooper approval, and controller verification of that evidence before mutation. The attested component set and inner payload include the current/new fixed-code/trust-set versions and the digest of every fixed file, including `release-trust.json`, so approval covers the exact bytes later staged on the Mac.

Infrastructure approval is a same-run GitHub Environment gate whose attestation binds the immutable inner payload and exact run attempt. Before that workflow is enabled, a separately authorized local administration preflight creates the environment, applies the reviewer/branch/bypass policy, records its immutable ID in reviewed trust configuration, and reads it back; workflow tokens cannot administer it. The approval job accepts only attempt 1, requires the run's review-history record to show `approved` for the pinned environment ID/name by Cooper's immutable GitHub user ID, rejects bypass-only completion, and snapshots the matching environment protection configuration. A failed or rerun infrastructure release requires a new commit and workflow run. Because this is currently a single-operator repository, self-review prevention would deadlock merge-triggered releases; administrator bypass is disabled, and adding another operator requires a distinct reviewer and self-review prevention.
...
A remotely managed Tunnel routes the three existing hostnames to pinned internal containers. Each origin has a separate internal network shared only with `cloudflared`; MCP reaches API through another explicit service network. Database access is limited to API, the dedicated scheduler, the one-shot migration job, and administrative backup/restore jobs. No database, API, MCP, web, Docker, SSH, or deployment port is published to the router or LAN.
...
Create compressed logical dumps hourly, at a frozen writer boundary before each eligible deployment, and after cutover or infrastructure upgrades. Before a deployment freeze, obtain a fresh signed witness observation of the current public stable digest; bind it into the frozen backup manifest, then require the independent monitor to accept that exact recovery set before migration or service replacement. A successful upload alone is not sufficient. Verify each dump, encrypt it with a unique data key wrapped by the escrowed backup key, and use a unique immutable object name. The bucket's default Compliance retention applies without trusting uploader-provided retention. The Mac credential cannot delete, overwrite, shorten retention, change policy, or disable Object Lock. The age of the latest independently accepted checkpoint must remain below one hour; backup duration, lock contention, or witness delay that threatens the RPO alerts and blocks deployment.
...
API and MCP health expose a non-secret build revision, and the web image exposes the same revision through a release-specific static version path and an `X-Build-Revision` header on every response. Cloudflare bypasses cache for revision paths and revision/health responses send `Cache-Control: no-store, max-age=0`; the verifier uses a nonce-bearing request and requires matching header/body revision plus an origin-fresh response. Verification must observe the expected candidate, not any healthy response.
...
MCP mutations generate or forward an `Idempotency-Key` to the public API. Authentication resolves a stable idempotency subject: the human user ID for interactive sessions, or a persisted credential/token-family ID for agents. An OAuth refresh carries forward the same family across replacement access and refresh-token rows; the current access-token row ID is never the idempotency subject. A database idempotency record is transactionally committed with the mutation and audit event under a unique `(idempotency subject, method, route, key)` scope. It stores a canonical request hash, completion state, and a typed, size-bounded reference to an allowlisted domain result or minimal non-secret receipt; arbitrary response bodies are never persisted. The same key and payload reconstructs the safe committed result; the same key with a different payload returns `409`; an uncommitted transaction leaves no success record and may be retried. Records outlive the maximum client/controller retry window, have an explicit expiry, and are pruned only under the global operation lock. This narrow database migration is explicitly in scope; an audit `request_id` alone is not an idempotency boundary.
...
`.github/workflows/deploy.yml` becomes one protected-main `push` workflow whose least-privilege CI jobs gate a SHA-bound release job. That job replaces AWS publication with GHCR images, the bundle, attestations, and monotonic successful-main publication. Infrastructure-class releases additionally gate a same-run, environment-protected approval-attestation job with `contents: read`, Actions/environment read, and its own attestation/OIDC permissions; a separate unprivileged job records hold-aware production observation. Pull-request workflows never receive release permissions, and no privileged follow-on event consumes artifacts from another run.

`compose.production.yaml`, versioned in the bundle, defines pinned PostgreSQL, one-shot migration, API, scheduler, MCP, web, and `cloudflared`; least-privilege database/service/per-origin networks; health/revision checks; Linux named storage; and delayed Tunnel startup until reconciliation. Before Docker Compose can resolve candidate input, the fixed verifier uses its installed root-owned validator, closed schema, policy, and compatibility manifest to parse the raw YAML with resource limits and reject aliases/merges, `build`, `include`, `extends`, `env_file`, providers, lifecycle hooks, configs, unknown keys, and all but an exact fixed secret name/path map. It then runs the installed manifest's pinned Compose version without credentials in an isolated directory and evaluates the fully resolved model against the installed service-specific allowlist. Candidate copies are CI/reference inputs only and cannot authorize themselves. The resolved gate rejects mutable tags, `privileged`, added capabilities, host PID/IPC/network namespaces, devices, Docker socket/API access, unapproved bind mounts, published ports, external networks, arbitrary secret paths, and unknown keys. Services use non-root users, `cap_drop: [ALL]`, `security_opt: [no-new-privileges:true]`, read-only filesystems, explicit writable tmpfs/volumes, and per-service secrets wherever compatible.
...
The API HTTP entrypoint never runs migrations or starts schedulers. The Mac uses separate one-shot migration, HTTP, and scheduler processes. During the AWS dual window, a distinct compatibility supervisor completes the same migration module before starting HTTP and scheduler lifecycles in the existing task shape; import-boundary and deployment-contract tests prevent migration logic from returning to the HTTP entrypoint, and the supervisor is removed with AWS.
...
`bootstrap.sh`, `launcher.sh`, `watch.sh`, `verify-release.sh`, `release-trust.json`, the Compose validator/schema/policy, and `runtime-compatibility.json` are installed as a checksummed fixed-code/trust set outside candidate-controlled paths and are not writable by the production runtime. Candidate files remain data until that installed verifier validates provenance, envelope structure, checksums, Compose policy, bootstrap compatibility, pinned approval trust, and the approved new fixed-set digest using only installed trust inputs. Any change to those inputs is an attended infrastructure release whose approval binds the old and new fixed-set digests and cross-version substitution/recovery evidence. A root-owned installer, invoked interactively from a separate Mac administrator identity, is the only writer: it stages the approved set, tests it against current and predecessor envelopes, atomically activates it with the prior set retained, and rolls back without executing candidate controller code on any failure. The production login has neither `sudo` nor write access to the installer or fixed-code/trust roots.
...
7. Controller records `deploying`, checks capacity, obtains a fresh signed witness observation bound to the current public stable digest, creates the bounded remote maintenance lease, disables Tunnel ingress, drains API/MCP/background work, and proves zero connections, transactions, queued jobs, and writers.
8. From that frozen point it creates, verifies, encrypts, and uploads the immutable backup/recovery manifest bound to the pre-freeze observation, then waits for independent acceptance of that exact recovery set; upload or acceptance failure restarts the unchanged stable release without mutation.
