...
CREATE FUNCTION public.finance_budget_policy_timezone(value text) RETURNS boolean
...
CREATE FUNCTION public.finance_budget_policy_object(value jsonb, keys text[]) RETURNS boolean
...
$$;
--> statement-breakpoint
CREATE FUNCTION public.finance_budget_policy_text(value jsonb, maximum integer, canonical boolean DEFAULT true) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE SECURITY INVOKER SET search_path = pg_catalog AS $$
...
$$;
--> statement-breakpoint
CREATE FUNCTION public.finance_budget_policy_number(value jsonb, minimum numeric, maximum numeric) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE SECURITY INVOKER SET search_path = pg_catalog AS $$
BEGIN
...
$$;
--> statement-breakpoint
CREATE FUNCTION public.finance_budget_policy_json_valid(value jsonb, kind text) RETURNS boolean
LANGUAGE plpgsql IMMUTABLE SECURITY INVOKER SET search_path = pg_catalog AS $$
...
BEGIN
  ...
  WHEN 'ref' THEN
    RETURN public.finance_budget_policy_object(value,ARRAY['id','revision']) AND public.finance_budget_policy_json_valid(value->'id','uuid') AND public.finance_budget_policy_text(value->'revision',200);
  WHEN 'timestamp' THEN
    IF jsonb_typeof(value) IS DISTINCT FROM 'string' THEN RETURN false; END IF;
    text_value := value #>> '{}';
  ...
  WHEN 'period' THEN
    IF NOT public.finance_budget_policy_object(value,ARRAY['from','through','timezone']) THEN RETURN false; END IF;
    ...
    d := (value->>'from')::date;
    RETURN (value->>'through') = to_char(d + interval '1 month - 1 day','YYYY-MM-DD') AND public.finance_budget_policy_timezone(value->>'timezone');
  WHEN 'directions', 'protections' THEN
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(value) LOOP
      IF NOT public.finance_budget_policy_object(item,CASE WHEN kind='directions' THEN ARRAY['allocationKey','direction'] ELSE ARRAY['allocationKey','minimumCents'] END) OR NOT public.finance_budget_policy_text(item->'allocationKey',120) THEN RETURN false; END IF;
      ...
      ELSIF NOT public.finance_budget_policy_number(item->'minimumCents',0,2147483647) THEN RETURN false;
      END IF;
    END LOOP;
    RETURN true;
  WHEN 'terms' THEN
    RETURN public.finance_budget_policy_object(value,ARRAY['currency','period','rollover','accounting','usageScope','perChangeCapCents','monthlyCapCents','expiresAt','baseline','directions','protections'])
      ...
      AND public.finance_budget_policy_json_valid(value->'period','period') AND public.finance_budget_policy_json_valid(value->'expiresAt','timestamp')
      AND public.finance_budget_policy_json_valid(value->'baseline','ref')
      AND public.finance_budget_policy_number(value->'perChangeCapCents',0,2147483647) AND public.finance_budget_policy_number(value->'monthlyCapCents',0,2147483647)
      AND public.finance_budget_policy_json_valid(value->'directions','directions') AND public.finance_budget_policy_json_valid(value->'protections','protections');
  WHEN 'plan' THEN
    IF NOT public.finance_budget_policy_object(value,ARRAY['userId','planId','revision','month','resources','allocations']) OR NOT public.finance_budget_policy_json_valid(value->'userId','uuid') OR NOT public.finance_budget_policy_json_valid(value->'planId','uuid') OR NOT public.finance_budget_policy_json_valid(value->'revision','ref') OR NOT coalesce(jsonb_typeof(value->'month')='string' AND value->>'month' ~ '^[0-9]{4}-(0[1-9]|1[0-2])$',false) THEN RETURN false; END IF;
    FOREACH key IN ARRAY ARRAY['resources','allocations'] LOOP
      ...
      FOR item IN SELECT * FROM jsonb_array_elements(collection) LOOP
        IF NOT public.finance_budget_policy_object(item,CASE WHEN key='resources' THEN ARRAY['key','kind','sourceId','amountCents'] ELSE ARRAY['key','kind','targetId','amountCents'] END) OR NOT public.finance_budget_policy_text(item->'key',120) OR NOT public.finance_budget_policy_number(item->'amountCents',0,2147483647) THEN RETURN false; END IF;
        IF item->>'key' = ANY(seen) THEN RETURN false; END IF; seen := array_append(seen,item->>'key');
        IF key='resources' THEN
          IF NOT coalesce(item->>'kind' IN ('income','reserve_draw','borrowing','other') AND jsonb_typeof(item->'kind')='string',false) OR NOT (item->'sourceId'='null'::jsonb OR public.finance_budget_policy_json_valid(item->'sourceId','uuid')) THEN RETURN false; END IF;
        ELSE
          IF NOT coalesce(item->>'kind' IN ('spending','savings','debt','goal','buffer') AND jsonb_typeof(item->'kind')='string',false) OR NOT (item->'targetId'='null'::jsonb OR public.finance_budget_policy_json_valid(item->'targetId','uuid')) THEN RETURN false; END IF;
        END IF;
    ...
    RETURN true;
  WHEN 'tuple' THEN
    IF NOT public.finance_budget_policy_object(value,ARRAY['userId','planId','policy','policyLifecycleRevision','profile','baseline','activeBudget','latestBudget','positionRevision','usageRevision']) OR NOT public.finance_budget_policy_json_valid(value->'userId','uuid') OR NOT public.finance_budget_policy_json_valid(value->'planId','uuid') OR NOT public.finance_budget_policy_json_valid(value->'policy','ref') OR NOT public.finance_budget_policy_json_valid(value->'baseline','ref') OR NOT public.finance_budget_policy_number(value->'policyLifecycleRevision',0,9007199254740991) THEN RETURN false; END IF;
    FOREACH key IN ARRAY ARRAY['profile','activeBudget','latestBudget'] LOOP
      IF NOT (value->key='null'::jsonb OR public.finance_budget_policy_json_valid(value->key,'ref')) THEN RETURN false; END IF;
    END LOOP;
    FOREACH key IN ARRAY ARRAY['positionRevision','usageRevision'] LOOP
      IF NOT (value->key='null'::jsonb OR public.finance_budget_policy_text(value->key,200,false)) THEN RETURN false; END IF;
    END LOOP;
    RETURN true;
  WHEN 'unavailable' THEN
    RETURN public.finance_budget_policy_object(value,ARRAY['state','reason']) AND value->'state'='"unavailable"'::jsonb AND jsonb_typeof(value->'reason')='string' AND value->>'reason' IN ('producer_not_registered','missing_evidence','stale_evidence');
  WHEN 'input' THEN
    IF NOT public.finance_budget_policy_object(value,ARRAY['evaluatedAt','terms','policyState','expected','observed','baseline','current','candidate','position','usage']) THEN RETURN false; END IF;
    RETURN public.finance_budget_policy_json_valid(value->'evaluatedAt','timestamp') AND public.finance_budget_policy_json_valid(value->'terms','terms')
      AND jsonb_typeof(value->'policyState')='string' AND value->>'policyState' IN ('draft','disabled','expired','unknown')
      AND public.finance_budget_policy_json_valid(value->'expected','tuple') AND public.finance_budget_policy_json_valid(value->'observed','tuple')
      AND (value->'baseline'='null'::jsonb OR public.finance_budget_policy_json_valid(value->'baseline','plan'))
      AND (value->'current'='null'::jsonb OR public.finance_budget_policy_json_valid(value->'current','plan'))
      AND public.finance_budget_policy_json_valid(value->'candidate','plan') AND public.finance_budget_policy_json_valid(value->'position','unavailable') AND public.finance_budget_policy_json_valid(value->'usage','unavailable');
  WHEN 'result' THEN
    IF NOT public.finance_budget_policy_object(value,ARRAY['kind','executionAvailable','executionUnavailableReasons','evaluatedAt','input','revisions','reasons','grossMovedCents','projectedMonthlyUsageCents','deltas']) THEN RETURN false; END IF;
    IF value->'kind' IS DISTINCT FROM '"denied"'::jsonb OR value->'executionAvailable' IS DISTINCT FROM 'false'::jsonb OR value->'executionUnavailableReasons' IS DISTINCT FROM '["authority_not_wired","position_commit_fence_not_wired"]'::jsonb OR NOT public.finance_budget_policy_json_valid(value->'input','input') OR value->'evaluatedAt' IS DISTINCT FROM value->'input'->'evaluatedAt' OR value->'revisions' IS DISTINCT FROM value->'input'->'observed' OR value->'projectedMonthlyUsageCents' IS DISTINCT FROM 'null'::jsonb THEN RETURN false; END IF;
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(value->'deltas') LOOP
      IF NOT public.finance_budget_policy_object(item,ARRAY['allocationKey','beforeCents','afterCents','deltaCents']) OR NOT public.finance_budget_policy_text(item->'allocationKey',120) OR NOT public.finance_budget_policy_number(item->'beforeCents',0,2147483647) OR NOT public.finance_budget_policy_number(item->'afterCents',0,2147483647) OR NOT public.finance_budget_policy_number(item->'deltaCents',-2147483647,2147483647) OR (item->>'deltaCents')::numeric <> (item->>'afterCents')::numeric-(item->>'beforeCents')::numeric THEN RETURN false; END IF;
    ...
    END LOOP;
    RETURN (value->'grossMovedCents'='null'::jsonb AND jsonb_array_length(value->'deltas')=0) OR (public.finance_budget_policy_number(value->'grossMovedCents',0,2147483647) AND (value->>'grossMovedCents')::numeric=positive);
  ELSE RETURN false;
  END CASE;
...
--> statement-breakpoint
CREATE FUNCTION public.finance_budget_policy_reject_snapshot_update() RETURNS trigger
...
$$;
--> statement-breakpoint
CREATE TRIGGER finance_budget_policy_versions_immutable BEFORE UPDATE ON finance_budget_policy_versions FOR EACH ROW EXECUTE FUNCTION public.finance_budget_policy_reject_snapshot_update();
--> statement-breakpoint
CREATE TRIGGER finance_budget_policy_previews_immutable BEFORE UPDATE ON finance_budget_policy_previews FOR EACH ROW EXECUTE FUNCTION public.finance_budget_policy_reject_snapshot_update();
--> statement-breakpoint
CREATE TRIGGER finance_budget_period_baselines_immutable BEFORE UPDATE ON finance_budget_period_baselines FOR EACH ROW EXECUTE FUNCTION public.finance_budget_policy_reject_snapshot_update();
--> statement-breakpoint
CREATE FUNCTION public.finance_budget_policy_guard_root_update() RETURNS trigger
LANGUAGE plpgsql SECURITY INVOKER SET search_path = pg_catalog AS $$
DECLARE allowed text[]; permitted boolean := false;
BEGIN
  IF TG_TABLE_NAME = 'finance_budget_policies' AND OLD.state = 'draft' THEN
    ...
    ELSIF NEW.state = 'disabled' THEN
      ...
      permitted := NEW.lifecycle_revision::bigint = OLD.lifecycle_revision::bigint + 1;
    END IF;
  ELSIF TG_TABLE_NAME = 'finance_budget_revision_proposals'
    AND OLD.state = 'inactive' AND NEW.state = 'withdrawn' THEN
    allowed := ARRAY['state','lifecycle_revision','updated_at','withdrawn_at','withdrawn_by_actor_id'];
    permitted := NEW.lifecycle_revision::bigint = OLD.lifecycle_revision::bigint + 1;
  END IF;
  IF NOT permitted OR (to_jsonb(NEW) - allowed) IS DISTINCT FROM (to_jsonb(OLD) - allowed) THEN
    RAISE EXCEPTION 'Budget policy root transition or immutable history violation' USING ERRCODE = '23514';
  END IF;
  RETURN NEW;
END;
$$;
--> statement-breakpoint
CREATE TRIGGER finance_budget_policies_guard_update BEFORE UPDATE ON finance_budget_policies FOR EACH ROW EXECUTE FUNCTION public.finance_budget_policy_guard_root_update();
--> statement-breakpoint
CREATE TRIGGER finance_budget_revision_proposals_guard_update BEFORE UPDATE ON finance_budget_revision_proposals FOR EACH ROW EXECUTE FUNCTION public.finance_budget_policy_guard_root_update();

--> statement-breakpoint
ALTER TABLE "finance_budget_policy_versions" ADD CONSTRAINT "finance_budget_policy_versions_contract_check" CHECK (public.finance_budget_policy_json_valid(directions,'directions') IS TRUE AND public.finance_budget_policy_json_valid(protections,'protections') IS TRUE AND public.finance_budget_policy_json_valid(jsonb_build_object('from',period_from,'through',period_through,'timezone',timezone),'period') IS TRUE);

--> statement-breakpoint
ALTER TABLE "finance_budget_policy_versions" ADD CONSTRAINT "finance_budget_policy_versions_scalar_contract_check" CHECK (public.finance_budget_policy_json_valid(to_jsonb(id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(user_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(policy_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(plan_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(baseline_budget_version_id),'uuid') IS TRUE AND isfinite(created_at) AND created_at>='0001-01-01T00:00:00Z'::timestamptz AND created_at<'10000-01-01T00:00:00Z'::timestamptz AND isfinite(expires_at) AND expires_at>='0001-01-01T00:00:00Z'::timestamptz AND expires_at<'10000-01-01T00:00:00Z'::timestamptz);

--> statement-breakpoint
ALTER TABLE "finance_budget_revision_proposals" ADD CONSTRAINT "finance_budget_revision_proposals_contract_check" CHECK (public.finance_budget_policy_json_valid(candidate_snapshot,'plan') IS TRUE AND candidate_snapshot->>'userId'=user_id::text AND candidate_snapshot->>'planId'=plan_id::text);

--> statement-breakpoint
ALTER TABLE "finance_budget_revision_proposals" ADD CONSTRAINT "finance_budget_revision_proposals_scalar_contract_check" CHECK (public.finance_budget_policy_json_valid(to_jsonb(id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(user_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(policy_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(policy_version_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(plan_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(baseline_budget_version_id),'uuid') IS TRUE AND (profile_version_id IS NULL OR (public.finance_budget_policy_json_valid(to_jsonb(profile_version_id),'uuid') IS TRUE)) AND (active_budget_version_id IS NULL OR (public.finance_budget_policy_json_valid(to_jsonb(active_budget_version_id),'uuid') IS TRUE)) AND (latest_budget_version_id IS NULL OR (public.finance_budget_policy_json_valid(to_jsonb(latest_budget_version_id),'uuid') IS TRUE)) AND isfinite(created_at) AND created_at>='0001-01-01T00:00:00Z'::timestamptz AND created_at<'10000-01-01T00:00:00Z'::timestamptz AND isfinite(updated_at) AND updated_at>='0001-01-01T00:00:00Z'::timestamptz AND updated_at<'10000-01-01T00:00:00Z'::timestamptz AND (withdrawn_at IS NULL OR (isfinite(withdrawn_at) AND withdrawn_at>='0001-01-01T00:00:00Z'::timestamptz AND withdrawn_at<'10000-01-01T00:00:00Z'::timestamptz)));

--> statement-breakpoint
ALTER TABLE "finance_budget_policy_previews" ADD CONSTRAINT "finance_budget_policy_previews_contract_check" CHECK (public.finance_budget_policy_json_valid(input_snapshot,'input') IS TRUE AND public.finance_budget_policy_json_valid(result_snapshot,'result') IS TRUE AND result_snapshot->'input'=input_snapshot AND (input_snapshot->>'evaluatedAt')::timestamptz=evaluated_at AND expires_at <= (input_snapshot->'terms'->>'expiresAt')::timestamptz AND input_snapshot->'observed'->>'userId'=user_id::text AND input_snapshot->'observed'->'policy'->>'id'=policy_version_id::text);

--> statement-breakpoint
ALTER TABLE "finance_budget_policy_previews" ADD CONSTRAINT "finance_budget_policy_previews_scalar_contract_check" CHECK (public.finance_budget_policy_json_valid(to_jsonb(id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(user_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(proposal_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(policy_version_id),'uuid') IS TRUE AND isfinite(created_at) AND created_at>='0001-01-01T00:00:00Z'::timestamptz AND created_at<'10000-01-01T00:00:00Z'::timestamptz AND isfinite(evaluated_at) AND evaluated_at>='0001-01-01T00:00:00Z'::timestamptz AND evaluated_at<'10000-01-01T00:00:00Z'::timestamptz AND isfinite(expires_at) AND expires_at>='0001-01-01T00:00:00Z'::timestamptz AND expires_at<'10000-01-01T00:00:00Z'::timestamptz);

--> statement-breakpoint
ALTER TABLE "finance_budget_period_baselines" ADD CONSTRAINT "finance_budget_period_baselines_contract_check" CHECK (public.finance_budget_policy_json_valid(jsonb_build_object('from',period_from,'through',period_through,'timezone',timezone),'period') IS TRUE);

--> statement-breakpoint
ALTER TABLE "finance_budget_period_baselines" ADD CONSTRAINT "finance_budget_period_baselines_scalar_contract_check" CHECK (public.finance_budget_policy_json_valid(to_jsonb(id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(user_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(plan_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(budget_version_id),'uuid') IS TRUE AND isfinite(confirmed_at) AND confirmed_at>='0001-01-01T00:00:00Z'::timestamptz AND confirmed_at<'10000-01-01T00:00:00Z'::timestamptz);

--> statement-breakpoint
ALTER TABLE "finance_budget_policies" ADD CONSTRAINT "finance_budget_policies_scalar_contract_check" CHECK (public.finance_budget_policy_json_valid(to_jsonb(id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(user_id),'uuid') IS TRUE AND public.finance_budget_policy_json_valid(to_jsonb(plan_id),'uuid') IS TRUE AND isfinite(created_at) AND created_at>='0001-01-01T00:00:00Z'::timestamptz AND created_at<'10000-01-01T00:00:00Z'::timestamptz AND isfinite(updated_at) AND updated_at>='0001-01-01T00:00:00Z'::timestamptz AND updated_at<'10000-01-01T00:00:00Z'::timestamptz AND (disabled_at IS NULL OR (isfinite(disabled_at) AND disabled_at>='0001-01-01T00:00:00Z'::timestamptz AND disabled_at<'10000-01-01T00:00:00Z'::timestamptz)));
