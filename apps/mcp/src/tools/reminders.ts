...
const id = idSchema.describe("nohmi reminder identifier");
...
const directMutation =
  "This is a direct, audited mutation. nohmi's API scopes and scoped-agent policy decision are authoritative; profile preferences and MCP annotations do not grant access.";
...
export function registerReminderTools(server: McpServer, api: PersonalOsApiClient) {
  ...
  server.registerTool(
    "create_reminder_attention_item",
    {
      annotations: annotations.create,
      description:
        "Create or refresh one open important, upcoming, or follow-up item for an owned active Reminder. nohmi locks and validates the Reminder, derives its local source and current revision, and deduplicates the open Reminder/kind pair.",
      inputSchema: z.object({
  ...
  server.registerTool(
    "delete_reminder",
    {
      ...
      inputSchema: z.object({ expectedUpdatedAt: isoDateTime, id }),
