...
**Status:** Specification and rollout approved by Cooper on 2026-09-14. Milestone completion still
requires exact merge and verification evidence.
...
1. Publish the approved spec/plan to main under COO-45 through the repository PR workflow.
   A temporary baseline-delivery task ports only the reviewed documentation, then the COO-46
   implementation, in two independent PRs from fresh main. The reviewed source is
   `cooper/finance-review-context` at `d309380075d0d881388671df8ece26b57d25a010`; this bounded
   baseline-port exception does not permit copying the whole branch. COO-45 remains open for
   orchestration after its documentation PR lands.
...
Cooper's 2026-09-14 approval authorizes the two baseline prerequisite PRs through gated squash
merges using `ship-it`. Later milestones follow the applicable relayed merge authority and
repository workflows. No manual deployment or production Finance/SMS mutation is authorized.
...
document does not itself create monitoring automations or implementation tasks.
