  ...
  return {
    async createForRun(userId: string, runId: string): Promise<FinancePeriodReview> {
      const [existing] = await db
        ...
        .where(and(eq(financePeriodReviews.runId, runId), eq(financePeriodReviews.userId, userId)))
        .limit(1);
      ...
      const observed = await status.getFinanceStatus(userId, run.scope);
      if (
        observed.freshness.state !== "current" ||
        observed.freshness.blockers.length > 0 ||
        observed.details.rulebookVersion !== run.rulebookVersion
      )
        throw new AppError("conflict", "Finance verification is not current enough to publish.");
