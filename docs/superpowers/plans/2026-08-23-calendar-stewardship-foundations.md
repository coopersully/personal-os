...
- Existing migrations are immutable. Add one append-only `0070_calendar_stewardship_foundations.sql` migration and one journal entry, and exercise it against a fresh PostgreSQL database.
...
- `packages/database/migrations/0070_calendar_stewardship_foundations.sql` — append-only expansion.
- `packages/database/migrations/meta/_journal.json` — register migration 0070 without editing prior entries.
...
- Create (Integration): `packages/database/migrations/0070_calendar_stewardship_foundations.sql`
...
it("keeps Calendar findings stable and reviews immutable", async () => {
  ...
  const migrationSql = await readFile(
    resolve(process.cwd(), "packages/database/migrations/0070_calendar_stewardship_foundations.sql"),
    "utf8",
  );
...
Expected: FAIL because `calendarFindings`, `calendarReviews`, and migration 0070 do not exist.
...
Create migration 0070 with only the two tables, their foreign keys, checks, and indexes:
...
Append this exact journal record after index 69, using a monotonically greater `when` value:
...
```json
{
  ...
  "version": "7",
  "when": 1787566200000,
  "tag": "0070_calendar_stewardship_foundations",
  "breakpoints": true
}
```
...
Expected: PASS; the integration suite applies every migration through 0070 to a fresh PostgreSQL 17 container.

- [ ] **Step 5: Commit the durable ledger**

```bash
git add packages/database/src/schema.ts packages/database/src/schema.test.ts packages/database/migrations/0070_calendar_stewardship_foundations.sql packages/database/migrations/meta/_journal.json
...
Expected: PASS with PostgreSQL migration 0070 applied in integration tests.
...
1. `packages/database/src/schema.ts`, migration 0070, schema test, and migration journal registration.
