...
HTTPS provider traffic on TCP 443, iCloud Mail IMAP over TLS on TCP 993, and authenticated iCloud
Mail SMTP submission with STARTTLS on TCP 587. PostgreSQL remains in
dedicated private subnets and accepts traffic only from application tasks. The signed-in human Mail
surface may use the TCP 587 path for confirmed iCloud delivery; autonomous and MCP workflows cannot
send mail.
