...
const id = idSchema.describe("nohmi object identifier");
...
export function registerMailTools(server: McpServer, api: PersonalOsApiClient) {
  ...
  server.registerTool(
    "snooze_mail",
    {
      ...
      description:
        "Temporarily hide a cached conversation until a specified time. This changes nohmi's local snooze state, not provider mail.",
  ...
  server.registerTool(
    "create_mail_draft",
    {
      ...
      description:
        "Save a durable nohmi Mail draft after verifying the account, recipients, subject, and body from the user's instruction. This does not contact the provider. Use the returned draft ID and exact same fields with send_mail.",
  ...
  server.registerTool(
    "send_mail",
    {
      ...
      description:
        "Send a previously saved durable nohmi Mail draft through its connected provider. draftId is required and every account, thread, recipient, subject, and body field must exactly match create_mail_draft. This creates an open-world external side effect; never retry an uncertain result until the person inspects provider Sent Mail and reconciles the draft in nohmi.",
      inputSchema: z.object({
  ...
  server.registerTool(
    "create_mail_attention_item",
    {
      ...
      description:
        "Create or refresh one open important, upcoming, or follow-up attention item for an owned Mail conversation. nohmi derives the source reference from the thread and deduplicates the same open thread/kind pair.",
      inputSchema: z.object({
  ...
  server.registerTool(
    "review_mail_rule",
    {
      ...
      description:
        "Re-preview one saved Mail rule against the current bounded recent window. Returns the rule ID/version, exact thread IDs, due states, dates, truncation, and a fingerprint; it never changes mail or rule state. Activation is an interactive review action in nohmi Settings.",
      inputSchema: z.object({ id }),
  ...
  server.registerTool(
    "update_mail_rule",
    {
      ...
      description:
        "Revise or pause a Mail rule using optimistic version matching. Agent activation is intentionally unavailable: the person reviews and activates the saved rule in nohmi Settings. Pause an active rule before changing matching behavior.",
      inputSchema: z.object({
