...
test('runtime overlay contains only allocation-derived non-secret values', () => {
  ...
  assert.match(overlay, /^DATABASE_URL=postgres:\/\/personal_os:personal_os@127\.0\.0\.1:55438\/personal_os$/m);
  assert.match(overlay, /^GOOGLE_REDIRECT_URI=http:\/\/127\.0\.0\.1:8793\/v1\/connectors\/google\/callback$/m);
...
test('runtime overlay is atomically written with private permissions', async (t) => {
  ...
  assert.equal(await readFile(path.join(root, '.env.codex.local'), 'utf8'), buildRuntimeOverlay(allocation));
});

test('config output remains compatible with cooper-run labels', () => {
  assert.equal(formatAllocation(allocation), [
    '  Root:      /tmp/linked',
    '  Tier:      2',
...
test('status is non-allocating for an unallocated root', async () => {
  ...
  const code = await runManager(['status', '--root', '/tmp/linked'], {
    ...
    acquireAllocation: async () => { acquired = true; },
...
test('purge requires explicit data-loss acknowledgement before mutation', async () => {
  ...
  const code = await runManager(['purge', '--root', '/tmp/linked'], {
    ...
    removeOwnedDockerResources: async () => { removed = true; return { ok: true }; },
    reconcileRegistry: async () => [],
  });
  assert.equal(code, 2);
...
test('start uses the explicit Compose project and attached supervisor then clears process records', async (t) => {
  ...
  const code = await runManager(['start', '--root', root], {
    stdout: () => {},
    resolveRepositoryContext: async () => ({ root, registryDir: path.join(root, '.git/ilo-runtime') }),
    reconcileRegistry: async () => [],
    migrateLegacyTiers: async () => ({}),
    acquireAllocation: async () => current,
    probePort: async () => ({ available: true }),
    inspectOwnedDockerResources: async () => ({ ok: true, resources: { containers: [], networks: [], volumes: [] } }),
    execFile: async (file, args, options) => { commands.push({ file, args, options }); return { stdout: '' }; },
    spawnAttached: async () => 0,
    getAllocationForRoot: async () => ({ ...current, state: 'running', processes: { supervisor: { pid: 1 } } }),
    replaceAllocation: async (_context, value) => { replacements.push(value); return value; },
  });
  assert.equal(code, 0);
  assert.deepEqual(commands[0].args.slice(0, 9), [
    'compose', '-p', allocation.composeProject, '-f', `${root}/.codex/runtime/compose.yaml`, 'up', '-d', 'postgres',
  ]);
