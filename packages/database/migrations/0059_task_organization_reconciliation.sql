-- 0055_task_organization and 0055_finance_sync_health were published on parallel
-- branches with the same Drizzle timestamp. A database that has already applied
-- Finance will skip the parallel Task migration, so this append-only migration
-- applies the unchanged Task transition only when its first table is absent.
DO $reconcile$
DECLARE
	task_rows bigint;
