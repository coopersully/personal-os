...
- [ ] Before compatibility testing, generate a canonical candidate component-set descriptor containing exact API/MCP/web platform-image digests, PostgreSQL/cloudflared digests, migration journal and fingerprints, controller checksums, state-schema version, minimum bootstrap version, and the current/new fixed-code/trust-set versions plus checksums/digests for `bootstrap.sh`, `launcher.sh`, `watch.sh`, `verify-release.sh`, `release-trust.json`, and the root-owned installer. Its digest is the stable pre-envelope candidate identity: it contains no compatibility evidence, approval, inner-payload digest, or outer-envelope digest.
...
- [ ] Install `bootstrap.sh`, `launcher.sh`, `watch.sh`, `verify-release.sh`, and `release-trust.json` as a versioned, checksummed fixed-code/trust set outside bundle/controller paths, owned and permissioned so the production runtime cannot modify it. Before any candidate controller or Compose hook executes, fixed code must verify repository/workflow/ref/SHA provenance, outer and inner envelopes, component/evidence/checksum bindings, bootstrap compatibility, pinned environment approval policy, and the fully resolved Compose policy. Add hostile fixtures proving candidate scripts, trust-config substitutions, includes, profiles, providers, interpolations, hooks, and controller code remain inert until all fixed-code checks pass.
- [ ] Define the only fixed-code/trust update path as `install-fixed-code.sh`, a root-owned installer invoked interactively from a separate Mac administrator identity after the currently installed verifier validates the attested old/new fixed-set versions/digests and infrastructure approval. Stage the exact approved bytes—including `release-trust.json`—beside the current set, run old/new cross-version envelope and recovery tests in an isolated directory, atomically switch the fixed-code/trust pointer, retain the prior set, and automatically revert on startup/self-test failure. The production login has no `sudo` or write access to the installer/fixed roots, and a candidate bundle cannot directly replace fixed code or trust data.
