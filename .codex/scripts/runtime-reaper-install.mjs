...
export async function inspectInstalledReaper(options) {
  ...
  if (manifest.repositoryId !== options.repositoryId || manifest.gitCommonDir !== options.gitCommonDir) {
    return { status: 'identity-mismatch', manifest, ...paths };
  }
  const missing = [];
  for (const file of [...BUNDLE_FILES, 'manifest.json']) {
    try { await stat(path.join(paths.bundleDir, file)); } catch { missing.push(file); }
  }
  try { await stat(paths.plistPath); } catch { missing.push(path.basename(paths.plistPath)); }
  ...
  const current = manifest.protocolVersion === requestedProtocol
...
export async function installReaper(options) {
  ...
  if (inspected.status === 'identity-mismatch') throw new Error('Installed reaper repository identity does not match.');
  if (inspected.status === 'newer') throw new Error('Refusing to downgrade the installed reaper protocol or schema.');
  ...
  await writePrivate(path.join(paths.bundleDir, 'manifest.json'), `${JSON.stringify(manifestFor(options), null, 2)}\n`);
...
export async function uninstallReaper(options) {
  ...
  if (inspected.manifest?.repositoryId !== options.repositoryId || inspected.manifest?.gitCommonDir !== options.gitCommonDir) {
