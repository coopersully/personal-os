...
test('buildServiceSpecs derives every URL and binds all services to loopback', () => {
  ...
  assert.equal(specs.api.env.GOOGLE_REDIRECT_URI, 'http://127.0.0.1:8793/v1/connectors/google/callback');
  assert.equal(specs.api.env.X_REDIRECT_URI, 'http://127.0.0.1:8793/v1/x-bookmarks/callback');
  assert.equal(specs.api.env.DATABASE_URL, 'postgres://personal_os:personal_os@127.0.0.1:55438/personal_os');
...
test('child records its process group and all services only after all readiness probes pass', async () => {
  ...
  const result = await runSupervisor({
    ...
    readiness: async (name) => { probes.push(name); return true; },
    exitAfterReady: true,
  });
