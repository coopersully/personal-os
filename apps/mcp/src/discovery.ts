...
export function registerIloContextTool(server: McpServer, options: DiscoveryOptions): void {
  server.registerTool(
    ...
    {
      description:
        "Call this first. Identify the nohmi user, local time, setup readiness, granted capabilities, available tools, safe workflow stages, and links back to the nohmi work surface.",
      inputSchema: z.object({}),
      title: "Orient to nohmi",
    },
    async () =>
      apiResult(async () => ({
        ...
        mcp: {
          ...
          workflow: [
            ...
            "commit: perform an authorized mutation through nohmi's API",
            "verify: confirm the result from authoritative state or activity",
            "recover: use nohmi's reversible state and recovery links when needed",
          ],
        },
      })),
  );
...
export function registerIloDiscovery(server: McpServer, options: DiscoveryOptions): void {
  server.registerResource(
    ...
    {
      description:
        "Authenticated identity, local time, readiness, granted scopes, available MCP tools, and nohmi work-surface links.",
  ...
  server.registerResource(
    ...
    {
      description:
        "One self-contained nohmi setup step, including evidence, instructions, required capabilities, and the human approval boundary.",
      mimeType: "application/json",
      title: "nohmi setup step",
    },
    async (uri, variables) => {
  ...
  if ([...options.scopes].some((scope) => scope.endsWith(":read"))) {
    server.registerResource(
      ...
      {
        description:
          "The user's operative or draft guidance for one nohmi domain. Draft guidance is never execution authority.",
        mimeType: "application/json",
        title: "nohmi domain guidance",
      },
      async (uri, variables) => {
        const domain = assistantDomainSchema.parse(variables.domain);
        return {
...
function registerPrompts(
  ...
  const prompts = [
    {
      description: "Orient to nohmi, then continue the next incomplete setup step.",
      name: "set_up_ilo",
    ...
    },
    {
      ...
      text: canMaintainFinances
        ? "Call get_ilo_context and get_finance_status. State data freshness and uncertainty. When the caller intends maintenance, invoke maintain_finances once; nohmi keeps questions and approvals pending rather than guessing."
    ...
    {
      description: "Run a cross-domain weekly review grounded in nohmi's current state.",
      name: "weekly_review",
