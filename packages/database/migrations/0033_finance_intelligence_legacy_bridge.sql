-- A prior unpublished Pinterest branch used the migration slots that Finance
-- now occupies on main. Drizzle orders migrations by the journal timestamp, so
-- those existing local databases would otherwise skip 0016_finance_intelligence.
-- Fresh databases have already applied 0016 and skip this bridge.
DO $$
BEGIN
