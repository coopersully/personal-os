...
const id = z.string().uuid().describe("nohmi object identifier");
...
export function registerFinanceStewardshipTools(server: McpServer, api: PersonalOsApiClient) {
  ...
  server.registerTool(
    "set_finance_budget_plan",
    {
      ...
      description:
        "Set one complete monthly budget plan with its assumptions and rationale. nohmi returns applied, pending_review, or needs_input.",
  ...
  server.registerTool(
    "create_finance_attention_item",
    {
      ...
      description:
        "Create or refresh one open important, upcoming, or follow-up attention item for an owned Finance transaction. nohmi locks and validates the transaction, derives its source reference, and deduplicates the same open transaction/kind pair. Repeated calls refresh the item and advance its version.",
      inputSchema: z.object({
        transactionId: id,
  ...
  server.registerTool(
    "apply_finance_categorizations",
    {
      annotations: writeAnnotations,
      description:
        "Apply revision-guarded categorization decisions. nohmi requires evidence first, then either applies, queues one review, or asks a bounded question.",
  ...
  server.registerTool(
    "update_finance_recurring_obligation",
    {
      annotations: writeAnnotations,
      description:
        "Accept, pause, or cancel a recurring ledger obligation; nohmi returns its apply-or-review disposition.",
  ...
  server.registerTool(
    "resolve_finance_alert",
    {
      annotations: writeAnnotations,
      description:
        "Resolve or dismiss a Finance ledger alert; nohmi returns its apply-or-review disposition.",
  ...
  server.registerTool(
    "reconcile_finance_reimbursement",
    {
      ...
      description:
        "Create, evidence-match a ledger credit to, or cancel a reimbursement. This can change Finance projections and may require review; nohmi never executes an external payment.",
  ...
  server.registerTool(
    "set_finance_transaction_breakdown",
    {
      annotations: writeAnnotations,
      description:
        "Set exact category allocations for one posted transaction. Allocations are one-off unless the optional futureRule explicitly requests an evidence-backed, consequential reusable merchant rule; nohmi returns its apply-or-review disposition.",
  ...
  server.registerTool(
    "update_finance_income_stream",
    {
      annotations: writeAnnotations,
      description:
        "Accept or pause an inferred income stream; nohmi returns its apply-or-review disposition.",
  ...
  server.registerTool(
    "update_finance_profile",
    {
      annotations: writeAnnotations,
      description:
        "Update the financial planning baseline; nohmi returns its apply-or-review disposition.",
  ...
  server.registerTool(
    "refresh_finance_insights",
    {
      annotations: writeAnnotations,
      description:
        "Refresh recurring, income, and alert ledger insights through nohmi's apply-or-review flow.",
      inputSchema: z.object({}),
